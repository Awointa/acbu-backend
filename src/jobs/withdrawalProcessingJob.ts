  ...
  ch.consume(
    ...
    async (msg: ConsumeMessage | null) => {
      ...
      try {
        ...
        if (!tx || tx.type !== 'burn') {
          logger.warn('Withdrawal job: transaction not found or not burn', { transactionId });
          ch.ack(msg);
          return;
        }
        if (tx.status !== 'processing' && tx.status !== 'pending') {
          logger.debug('Withdrawal job: transaction already finalized', { transactionId, status: tx.status });
          ch.ack(msg);
          return;
        }
        ...
        if (!currency || !Number.isFinite(amount) || amount <= 0 || !recipientJson) {
          ...
          logger.warn('Withdrawal job: missing currency, amount or recipient', { transactionId });
          ch.ack(msg);
          return;
        }
        ...
        try {
          ...
          const result = await provider.disburseFunds(amount, currency, recipient);
          await prisma.transaction.update({
            where: { id: transactionId },
            data: {
              status: 'completed',
          ...
          await publishWithdrawalNotification(transactionId, tx.userId, 'completed', currency, amount);
        } catch (err) {
          logger.error('Withdrawal disbursement failed', { transactionId, error: err });
          await prisma.transaction.update({
            where: { id: transactionId },
            data: { status: 'failed' },
          });
          await publishWithdrawalNotification(transactionId, tx.userId, 'failed', currency, amount);
...
async function publishWithdrawalNotification(
  ...
  try {
    ...
    logger.warn('Failed to publish withdrawal notification', { transactionId, error: e });
  }
}
...
export async function enqueueWithdrawalProcessing(payload: WithdrawalPayload): Promise<void> {
  ...
  logger.info('Withdrawal processing enqueued', { transactionId: payload.transactionId });
}
