...
export async function assignValidators(applicationId: string): Promise<void> {
  ...
  if (!app || app.status !== 'awaiting_review') {
    logger.warn('assignValidators: app not in awaiting_review', { applicationId });
    return;
  }
  const existing = await prisma.kycValidation.count({ where: { applicationId } });
  ...
  logger.info('KYC validators assigned', { applicationId, count: selected.length });
}
...
export async function getTasksForValidator(validatorUserId: string): Promise<
  { validationId: string; applicationId: string; machineRedactedPayload: object }[]
> {
...
async function tryResolveApplication(applicationId: string): Promise<void> {
  ...
  const approved =
    rule === 'all_approve'
      ...
      : results.filter((r: string) => r === 'approve').length > results.length / 2;
  const { approveApplication, rejectApplication } = await import('./applicationService');
  if (approved) {
    await approveApplication(applicationId);
  } else {
    await rejectApplication(applicationId, 'Human validation consensus: reject');
  }
}
